...
export class DeliverymanComponent implements OnInit {
  ...
  ingresar() {
    if(this.form.valid){ 
      this.repartidor = this.deliveryManService.getRepartidorLogin(this.form.value.cedula,this.form.value.pass).subscribe((data : any) => {
        this.dato = data;
        
        if(this.dato.length > 0){
          console.log(this.dato);
          this.deliveryManService.setdeliverIdStorage(this.dato[0]["cedula"]);
