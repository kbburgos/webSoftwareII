...
export class NoveltyService {
   ...
   * @version 1.0.0
   * @returns {JSON} JSON with the data obtained from API.
   * @desc This method is responsible for searching all novelty of a custumer reported. <br> Creation Date: 08/29/2020
   * @type {Promise<void>} Void type promise.
   * @author Brenda Bermello <bremiber@espol.edu.ec>
   */
  getnovedadesReportado(token: string, cedula: string) {
    let headers = {
      "Content-Type": "application/json",
      Authorization: "Bearer " + token,
    };
